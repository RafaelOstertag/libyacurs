...
class MainWindow : public YACURS::Window {
    private:
        ...
        YACURS::Label* label1;
        YACURS::MessageBox2* messagebox2;
    ...
    protected:
        void window_close_handler(YACURS::Event& _e) {
            ...
            if (messagebox2 != 0 && evt.data() == messagebox2) {
                YACURS::Curses::statusbar()->push("MessageBox2 closed");

                if (messagebox2->dialog_state() == YACURS::DIALOG_OK)
        ...
        void button_press_handler(YACURS::Event& _e) {
            ...
            if (e.data() == button1) {
                assert(messagebox2 == 0);

#ifdef YACURS_USE_WCHAR
                messagebox2 = new YACURS::MessageBox2("«Test MessageBox2»",
