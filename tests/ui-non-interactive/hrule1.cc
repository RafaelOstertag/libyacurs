// $Id$
//
// Test basic functionality of HRule
#ifdef HAVE_CONFIG_H
#include "config.h"
#endif
...
void alrm(YACURS::Event& _e) {
    ...
    if (ws.ws_row <= YACURS::MIN_ROWS || ws.ws_col <= YACURS::MIN_COLS)
...
int main() {
    ...
    try {
        ...
        YACURS::TitleBar* title = new YACURS::TitleBar(
            YACURS::TitleBar::POS_TOP, "Horizontal Rule 1");
        ...
        YACURS::HRule* hrule = new YACURS::HRule();
        YACURS::Label* label2 = new YACURS::Label("Test label2");

        vpack->add_back(label1);
        vpack->add_back(hrule);
