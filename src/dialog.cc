...
class HotKeyEsc : public HotKey {
   ...
   public:
    HotKeyEsc(Dialog& d) : HotKey(27), __dialog(d) {}

    HotKeyEsc(const HotKeyEsc& hk) : HotKey(hk), __dialog(hk.__dialog) {}
...
// Private
//
Dialog::Dialog(const Dialog&) { throw EXCEPTIONS::NotSupported(); }
...
// Protected
//
const Button* const Dialog::ok_button() const { return __bok; }

const Button* const Dialog::cancel_button() const { return __bcancel; }

const Button* const Dialog::yes_button() const { return __byes; }

const Button* const Dialog::no_button() const { return __bno; }

void Dialog::dialog_state(DIALOG_STATE st) { __dstate = st; }

void Dialog::add_button(Button* b) { __hpack->add_back(b); }

void Dialog::add_button(Spacer* s) { __hpack->add_back(s); }

void Dialog::button_press_handler(Event& _e) {
...
Dialog::Dialog(const std::string& _title, DIALOG_TYPE _dt, DIALOG_SIZE _ds)
    : __vpack(new VPack),
      __hpack(new HPack),
      __byes(0),
      __bok(0),
...
Dialog::~Dialog() {
    ...
    if (__bok) delete __bok;
    if (__byes) delete __byes;
    if (__bcancel) delete __bcancel;
    if (__bno) delete __bno;
...
DIALOG_STATE
Dialog::dialog_state() const { return __dstate; }

DIALOG_TYPE
Dialog::dialog_type() const { return __dialog_type; }

void Dialog::title(const std::string& _title) {
    __title = _title;
...
std::string Dialog::title() const { return __title; }
